 ...
 * @privateinitializer
 */
__publicField(BigNumber, "groupSizes", [
  0,
  0,
  25,
  16,
 ...
 * @privateinitializer
 */
__publicField(BigNumber, "groupBases", [
  0,
  0,
  33554432,
  43046721,
 ...
 */
__publicField(BigNumber, "wordSize", 26);
__publicField(BigNumber, "WORD_SIZE_BIGINT", BigInt(_BigNumber.wordSize));
__publicField(BigNumber, "WORD_MASK", (1n << _BigNumber.WORD_SIZE_BIGINT) - 1n);
__publicField(BigNumber, "MAX_SAFE_INTEGER_BIGINT", BigInt(Number.MAX_SAFE_INTEGER));
__publicField(BigNumber, "MIN_SAFE_INTEGER_BIGINT", BigInt(Number.MIN_SAFE_INTEGER));
__publicField(BigNumber, "MAX_IMULN_ARG", 67108864 - 1);
__publicField(BigNumber, "MAX_NUMBER_CONSTRUCTOR_MAG_BIGINT", (1n << 53n) - 1n);
...
var JacobianPoint = class extends BasePoint {
  /**
...
var Point = class extends BasePoint {
  /**
   * @constructor
  ...
  static fromDER(bytes2) {
    ...
    if ((bytes2[0] === 4 || bytes2[0] === 6 || bytes2[0] === 7) && bytes2.length - 1 === 2 * len) {
      ...
      const res = new Point(bytes2.slice(1, 1 + len), bytes2.slice(1 + len, 1 + 2 * len));
      return res;
    } else if ((bytes2[0] === 2 || bytes2[0] === 3) && bytes2.length - 1 === len) {
      return Point.fromX(bytes2.slice(1, 1 + len), bytes2[0] === 3);
    }
  ...
  static fromJSON(obj, isRed) {
    ...
    const obj2point = (obj2) => {
      return new Point(obj2[0], obj2[1], isRed);
    };
    const pre = obj[2];
    res.precomputed = {
      beta: null,
  ...
  _getBeta() {
    ...
    if (pre != null) {
      ...
      const endoMul = (p) => {
        ...
        return new Point(p.x.redMul(curve2.endo.beta), p.y);
      };
      pre.beta = beta;
      beta.precomputed = {
        beta: null,
  ...
  _getEndomorphism(conf) {
    ...
    if (conf.lambda !== void 0) {
      ...
      if (gMulX != null && gXRedMulBeta != null && gMulX.cmp(gXRedMulBeta) === 0) {
        ...
        Curve.assert(gMulX2.cmp(gXRedMulBeta2) === 0, "Lambda selection does not match computed beta.");
      }
    }
    let basis;
...
var Signature = class {
  ...
  static fromDER(data, enc) {
    ...
    return new Signature(new BigNumber(r2), new BigNumber(s2));
  }
  /**
   * Takes an array of numbers or a string and returns a new Signature instance.
...
var PublicKey = class extends Point {
  /**
...
var PrivateKey = class extends BigNumber {
  /**
   ...
   */
  static fromRandom() {
    return new PrivateKey(Random_default(32));
  }
  /**
  ...
  deriveChild(publicKey, invoiceNumber, cacheSharedSecret, retrieveCachedSharedSecret) {
    ...
    const hmac2 = sha256hmac(sharedSecret.encode(true), invoiceNumberBin);
    const curve2 = new Curve();
    return new PrivateKey(this.add(new BigNumber(hmac2)).mod(curve2.n).toArray());
  }
  /**
...
var SymmetricKey = class extends BigNumber {
  /**
...
var _TransactionSignature = class extends Signature {
...
__publicField(TransactionSignature, "SIGHASH_ALL", 1);
__publicField(TransactionSignature, "SIGHASH_NONE", 2);
__publicField(TransactionSignature, "SIGHASH_SINGLE", 3);
__publicField(TransactionSignature, "SIGHASH_FORKID", 64);
__publicField(TransactionSignature, "SIGHASH_ANYONECANPAY", 128);
...
var Script = class {
  ...
  static fromHex(hex) {
    ...
    return Script.fromBinary(toArray2(hex, "hex"));
  }
  /**
   * @method fromBinary
...
var MerklePath = class {
   ...
   */
  static fromHex(hex) {
    return MerklePath.fromBinary(toArray2(hex, "hex"));
  }
...
var Transaction = class {
  ...
  static fromEF(ef) {
    ...
    for (let i = 0; i < inputsLength; i++) {
      ...
      const sourceTransaction = new Transaction(void 0, [], [], void 0);
   ...
   */
  static fromHex(hex) {
    return Transaction.fromBinary(toArray2(hex, "hex"));
  }
  /**
  ...
  static fromHexEF(hex) {
    return Transaction.fromEF(toArray2(hex, "hex"));
  }
  /**
  ...
  static fromHexBEEF(hex, txid) {
    return Transaction.fromBEEF(toArray2(hex, "hex"), txid);
  }
  /**
...
var defaultFetch2 = typeof window !== "undefined" ? fetch.bind(window) : fetch;
...
// main.ts
var WalletNotePlugin = class extends import_obsidian.Plugin {
  async onload() {
    this.store = new LocalKVStore(void 0, "notes");
    this.addCommand({
  ...
  async saveActiveFile() {
    ...
    if (!(file instanceof import_obsidian.TFile)) {
      new import_obsidian.Notice("No active file to save.");
      return;
    }
    const content = await this.app.vault.read(file);
    try {
      await this.store.set(file.path, content);
      new import_obsidian.Notice("Note saved to wallet.");
    } catch (e) {
      console.error("Saving note failed", e);
      new import_obsidian.Notice("Failed to save note.");
    }
  }
  async loadActiveFile() {
    const file = this.app.workspace.getActiveFile();
    if (!(file instanceof import_obsidian.TFile)) {
      new import_obsidian.Notice("No active file to load.");
      return;
    }
    try {
      ...
      if (typeof value === "string") {
        ...
        new import_obsidian.Notice("Note loaded from wallet.");
      } else {
        new import_obsidian.Notice("No note stored for this file.");
      }
    } catch (e) {
      console.error("Loading note failed", e);
      new import_obsidian.Notice("Failed to load note.");
    }
  }
};
